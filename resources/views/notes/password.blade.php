...
@section('content')
    <section class="auth-card compact">
        ...
        <div class="title-block">
            <h2>{{ $note->password ? 'Change note password' : 'Lock this note' }}</h2>
            <p class="muted">Each protected note uses its own password, separate from the account password.</p>
        </div>

        <form class="auth-form" method="POST" action="{{ Route::has('notes.password.update') ? route('notes.password.update', $note->id) : '#' }}" data-loading-form>
            @csrf
            @if ($note->password)
                <div class="field">
